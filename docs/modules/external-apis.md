...
| TR ID | 용도 | 소유자 | 비고 |
|-------|------|--------|------|
| H0STCNT0 | 실시간 시세 (체결가) | PriceSync | 최대 40종목 |
| H0STCNI9 | 실시간 체결 통보 | Execution | 보유 종목 체결 |
| H0STASP0 | 실시간 호가 (선택) | PriceSync | 선택적 구독 |

**중요**: 동일 WS 연결이지만 TR별로 다른 모듈이 소유
- PriceSync: 시세 데이터 수신 및 처리
- Execution: 체결 통보 수신 및 포지션 동기화
